...
def spawn_sandbox(mycommand, opt_name=None, **keywords):
	...
	if not opt_name:
		opt_name = os.path.basename(mycommand.split()[0])
	args.append(mycommand)
	print('SBargs: {}\topt_name: {}'.format(args, opt_name))
	print('kwargs: {}'.format(keywords))
	args[0] = '/usr/bin/fuseboxing'
	# FIXME: need root priviledge to chroot in sandbox process
	del keywords['uid']
	del keywords['gid']
	del keywords['groups']
	del keywords['umask']
	print('@@@ Launch Fusebox @@@')
	return spawn(args, opt_name=opt_name, **keywords)
